...
async function getAlbum(){
    ...
    const sql = `SELECT album_id, album_name, artist_img, album.status FROM album, artist WHERE album.artist_id = artist.artist_id AND album.status = 'active'`
    try {
...
async function getAlbumbyID( {album_id} ){
    const con = await connect()
    const sql = `SELECT album_id, album_name, artist_img, album.status FROM album, artist WHERE album.artist_id = artist.artist_id AND album.album_id = $1 AND album.status = 'active'`
